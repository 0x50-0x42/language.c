...
void _qsort(char **lines, int field, int left, int right, int (*comp)(void*, void*)) {
	if(left >= right)
		return;
	...
	_qsort(lines, field, left, swappedIdx - 2, comp);
	_qsort(lines, field, swappedIdx, right, comp);
}
